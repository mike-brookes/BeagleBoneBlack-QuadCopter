...
L3GD20H::L3GD20H( unsigned char _deviceAddress, int _busId )
{
    ...
    if ( setPower( _switch::ON ) < 0 ) {
        throw new i2cSetupException( "Critical Error : L3GD20H - Attempted to set the Gyroscopes Power state but this action failed." );
    }
}
...
{
    L3GD20H* gyro = ( L3GD20H* ) static_inst;
    int startTime;
    while ( 1 ) {
...
int L3GD20H::start()
{
    ...
    threadRet = pthread_create( &threadHandle, NULL, L3GD20H::runMainSensorUpdateThread, this );
